...
Along the indetermination gradient, when considering both general taxa-association table and observed inventory the diversity was increasingly overestimated (Fig. \@ref(fig:Fig1)*(a)*).
...
When only considering the general taxa-association table (Fig. \@ref(fig:Fig1)*(b)*) the richness ($q=0$) was underestimated (reaching a 50% underestimation), while both Shannon and Simpson diversities were overestimated (respectively reaching underestimations of 67% and 125%).

When only confidering the observed inventory (Fig. \@ref(fig:Fig1)*(c)*) the estimator remained slightly biased but it did not exceed 15% of the real diversity for any order of diversity.
...
mtext("(a) Only Expert Prior",at=0.12,line=-0.6,outer=TRUE, cex=0.9)
mtext("(b) Expert prior & observation",at=0.5,line=-0.6,outer=TRUE, cex=0.9)
mtext("(c) Only observation",at=0.8,line=-0.6,outer=TRUE, cex=0.9)
...
Along the sampling effort gradient from 500 to 22 000 trees, the richness estimation remained underestimated but the estimator confidence interval did not exceed 7%.
The Shannon and Simpson were less biased, for 2 000 trees inventoried the Shannon diversity bias fell to 15% while the bias of Simpson estimator fell to 6% (Fig. \@ref(fig:Fig3).
...
mtext("(a) Richness",at=0.12,line=-1,outer=TRUE)
mtext("(b) Shannon",at=0.45,line=-1,outer=TRUE)
mtext("(c) Simpson",at=0.8,line=-1,outer=TRUE)
mtext("Number of sampled trees",1,adj=1,line=-0.5,cex=0.8,outer=TRUE)
mtext("Estimation bias (%)",side=2,at=0.75,outer=T,line=-0.25, cex=0.8)
mtext("Equivalent Diversity",side=2,at=0.3,outer=T,line=-0.25, cex=0.8)
```
